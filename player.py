...
class Player:
	...
	def __init__(self, goal,states, actions, Q, pi, start_state, alpha=0.4, decay = .9999954, gamma = .9, epsilon=.4):  #T, cool_down, subField, partition_r,
	...
	def updateQ(self, initialState, finalState, a, o, reward, restrictActions=None):
		
		self.Q[initialState, a, o] = (1 - self.alpha) * self.Q[initialState, a, o] + \
	...
	def updatePolicy(self, state, retry=False):
		...
		if res.success: # and self.satisfyBound(res.x[1:])
			...
			#print(self.modifyProb(res.x[1:], state))
